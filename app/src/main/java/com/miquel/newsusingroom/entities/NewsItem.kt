...
suspend fun getNewsList(rssUrl: String): MutableList<NewsItem> {
    ...
    withContext(Dispatchers.IO) { // Mover la operación de red fuera del hilo principal
        ...
        for (i in 0 until items.length) {
            ...
            newsItems.add(
                NewsItem(title=title,link=link,date=date,content=content,author=author,
                    imageUrl = imageUrl?: "https://upload.wikimedia.org/wikipedia/commons/0/0a/No-image-available.png"
                )
            )
        }
    }
